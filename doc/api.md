...
> minimal fallback so tooling and CI can import it. `GET /health` answers `200` with
> `{"status": "unconfigured"}` (the container healthcheck depends on this); every
> other route returns `503` with a clear "not configured" message.
