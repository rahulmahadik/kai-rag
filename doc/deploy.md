...
| Database         | Postgres 14+ with `pgvector` 0.7+ (HNSW and `halfvec` are required; the `pgvector/pgvector` image bundles it) |
...
The simplest path, identical on a dev laptop and a server, and what the live server
runs. `run/setup.py` is a convenience wrapper around the same API that CI and the
test suite exercise directly. Needs Python 3.12+, a reachable Postgres, and your
LLM/embedding endpoint (e.g. Ollama with the models pulled).
...
> **Note on the Docker path.** CI builds this image on every push, boots it, and
> checks that it serves `/health` and runs as an unprivileged user. Compose itself
> (the multi-service bring-up) is not exercised in CI; if you hit a Compose-specific
> snag, please file an issue.
...
- **Local files / PDFs**, `SOURCE_DIR`. The bundled `samples/` are excluded from
  the Docker image (see `.dockerignore`), so under Compose mount your own
  directory and point `SOURCE_DIR` at the mount path.
...
it. Set `CHAT_PLATFORM` (`webex` | `slack` | `teams`), the platform tokens, and
